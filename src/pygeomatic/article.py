...
result is exactly the `{label}(command)` article format the tinyvolt-web
InteractivePage already renders. The span scanner mirrors tinyvolt-web's
`extractCommands.ts`: brace/paren depth counters (labels may contain LaTeX
...
# ---------------------------------------------------------------------------
# Span scanner (port of tinyvolt-web extractCommands.ts)
# ---------------------------------------------------------------------------
